...
@app.route('/')
def index() -> str:
    # Devuelve una cadena que representa un formulario HTML
    return """
    <form method="POST" action="/upload" enctype="multipart/form-data">
        <!-- Campo de entrada para el nombre 'datos' -->
...
# Definición de la ruta '/upload' con método POST
@app.route('/upload', methods=['POST'])
def upload(csvdata, bucketname, blobname):
    # Crear una instancia del cliente de almacenamiento de Google Cloud
    client = storage.Client()

    # Obtener el cubo (bucket) especificado por 'bucketname'
    bucket = client.get_bucket(bucketname)

    # Crear un objeto Blob en el cubo con el nombre 'blobname'
    blob = bucket.blob(blobname)

    # Cargar los datos CSV desde una cadena en el objeto Blob
    blob.upload_from_string(csvdata)

    # Construir la ubicación de GCS (Google Cloud Storage) para el archivo cargado
    gcslocation = 'gs://{}/{}'.format(bucketname, blobname)

    # Registrar un mensaje informativo indicando que la carga fue exitosa
    logging.info('Uploaded {} ...'.format(gcslocation))

    # Devolver la ubicación de GCS como respuesta
    return gcslocation


# ----------------------------------------------------

# Manejador de errores para el código de estado 500 (Internal Server Error)
@app.errorhandler(500)
def server_error(e: Union[Exception, int]) -> str:
    # Registrar una excepción en los registros de la aplicación
    logging.exception('An error occurred during a request.')

    # Devolver una respuesta HTML indicando el error interno y sugiriendo revisar los registros
    return """
...
def certs():
    ...
    # Variable global para almacenar los certificados, inicializada como None
    global CERTS

    # Si CERTS es None (es decir, no se ha cargado antes)
    if CERTS is None:
        # Realizar una solicitud GET a la URL que proporciona los certificados
        response = requests.get(
            'https://www.gstatic.com/iap/verify/public_key'
        )

        # Almacenar el resultado JSON en la variable global CERTS
        CERTS = response.json()
...
def get_metadata(item_name):
    ...
    # Endpoint base para la obtención de metadatos de Google Cloud
    endpoint = 'http://metadata.google.internal'

    # Construir la ruta completa para el item_name en los metadatos del proyecto
    ...
    # Obtener el valor del metadato como una cadena
    metadata = response.text

    # Devolver el valor del metadato
    return metadata
...
def audience():
    ...
    """
    # Variable global para almacenar el valor de audiencia
    global AUDIENCE

    # Si AUDIENCE es None (es decir, no se ha calculado antes)
    if AUDIENCE is None:
        # Obtener el número de proyecto y el ID del proyecto desde los metadatos
        project_number = get_metadata('numeric-project-id')
        project_id = get_metadata('project-id')

        # Construir el valor de audiencia utilizando el número de proyecto y el ID del proyecto
        AUDIENCE = '/projects/{}/apps/{}'.format(
            project_number, project_id
        )

    # Devolver el valor de audiencia
    return AUDIENCE
...
def validate_assertion(assertion):
    ...
    try:
        # Decodificar la assertion JWT utilizando la función jwt.decode
        info = jwt.decode(
            assertion,
            certs(),
            algorithms=['ES256'],
        ...
        # Extraer y devolver el correo electrónico y el ID del usuario
        return info['email'], info['sub']

    except Exception as e:
        # Manejar una excepción en caso de que la validación falle
        print('Failed to validate assertion: {}'.format(e), file=sys.stderr)

        # Devolver None para el correo electrónico y el ID del usuario en caso de error
        return None, None
...
def download_excel(df_v, nombre='LogErrores'):
    # Guardar el DataFrame como un archivo Excel con el nombre proporcionado
    df_v.to_excel(nombre+'.xlsx', index=False)

    # Leer el contenido del archivo Excel recién creado
    with open(nombre+'.xlsx', 'rb') as file:
        contents = file.read()

    # Crear un botón de descarga en Streamlit para el archivo Excel
...
def say_hello():
    ...
    # Obtener la assertion JWT de los encabezados de la solicitud
    assertion = request.headers.get('X-Goog-IAP-JWT-Assertion')

    # Validar la assertion JWT y obtener el correo electrónico y el ID del usuario
    email, id = validate_assertion(assertion)

    # Construir una página HTML de saludo con el correo electrónico del usuario
    page = "<h1>Hello {}</h1>".format(email)

    # Devolver la página como respuesta a la solicitud GET
    return page
...
# ----------------------------------------------------
                ...
                def dona_plotly(producto='INSTALACIONES', col=col111, titulo=None, tamano_pantalla=(275, 275)):
                    ...
                    colores = ['#54278f', '#9e9ac8', '#f2f0f7']

                    # Calcular porcentajes y contar valores
                    total = sum(valores)
                    conteos = [str(valor) for valor in valores]
                    porcentajes = [
                        f'{(valor/total)*100:.1f}%' for valor in valores]

                    # Crear un gráfico de dona interactivo con Plotly
                    fig = go.Figure(data=[
                        go.Pie(
                            labels=etiquetas,
                            values=valores,
                    ...
                    # Configurar el diseño del legend
